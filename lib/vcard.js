...
/**
 * Decode Quoted-Printable encoded string (RFC 2045)
 * @param {string} str encoded string
 * @param {string} charset character set (default: ISO-8859-1)
 * @returns {string}
 */
function decodeQuotedPrintable(str, charset) {
    // Default to ISO-8859-1 if no charset specified
    charset = charset || 'ISO-8859-1';
    
    // Extract bytes from =XX sequences and ASCII characters
    var bytes = [];
    var i = 0;
    while (i < str.length) {
        if (str[i] === '=' && i + 2 < str.length) {
            ...
            if (/^[0-9A-F]{2}$/i.test(hex)) {
                ...
                // Soft line break (= not followed by hex) - skip it
                i++;
            }
        } else {
            // ASCII character
    ...
    if (charset.toUpperCase() === 'UTF-8' || charset.toUpperCase() === 'UTF8') {
        ...
        // ISO-8859-1 / Latin-1 decoding (default)
    ...
    for (var i = 0; i < count; i++) {
        ...
        if (isValueContinued(i)) {
            while (isValueContinued(i)) {
                // Remove only the first space/tab (RFC 5322 folding indicator)
                data += lines[i + 1].replace(/^[ \t]/, '');
                i++;
            }
        }
        ...
        // Decode Quoted-Printable if encoding is specified
        if (meta.encoding && meta.encoding.indexOf('QUOTED-PRINTABLE') !== -1) {
            var charset = meta.charset ? meta.charset[0] : 'ISO-8859-1';
